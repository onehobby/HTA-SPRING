...
public class MybatisUtil {
	 ...
	 *			void   commit()
	 *			void   rollback()
	 *			주의사항) 쿼리를 한 번 실행할 때 마다 SqlSessionFactory로부터 새로운 SqlSession객체를
	 *				     획득해서 사용하고, 폐기한다.
	 *			
	 * 		- SqlSessionFactory (DBCP라고 생각해도 된다.)
	 * 			쿼리실행에 필요한 SqlSession객체를 제공하는 객체다.
	 * 			애플리케이션이 실행되는 동안 딱 한번만 생성해서 계속 사용한다.
	 * 			sqlSession openSession()
	 * 				* 새로운 sqlSession객체를 반환한다.
	 * 			
	 * 		- SqlSessionFactoryBuilder
	 * 			mybatis 환경설정파일(/src/main/resources/mybatis/mybatis-config.xml)을 이용해서 SqlSessionFactory객체를 생성한다.
