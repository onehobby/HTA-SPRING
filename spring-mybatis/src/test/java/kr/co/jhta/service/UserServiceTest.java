 ...
 * 		- JUnit를 활용한 단위테스트 실행시 같이 실행되는 Helper클래스다.
 * 		- @ContextConfiguration으로 지정된 설정파일을 읽어서 객체를 생성/조립하는
 * 		   스프링 컨테이너를 생성한다.
 * 		- @Autowired 어노테이션을 사용해서 테스트 대상 객체를 주입받을 수 있도록 지원한다.
 * @Transactional
 * 		- 단위테스트 클래스에서 @Transactional 어노테이션을 부착하면 테스트 작업 과정에서 수행한
 * 		  insert/update/delete작업을 Database에 반영시키지 않고,
...
public class UserServiceTest {
	...
	public void testAddNewUser() {
		User user = new User("hong1234", "홍길동", "zxcv1234", "12345@12345");
	...
	public void testLogin() {
		User user = userService.login("hong", "1234");
	...
	public void testLoginFail() {
		userService.login("hong", "zxcv1234");
	}
